...
#define thread_mutex_init(pmtx,attr)              InitializeCriticalSection(pmtx)
#define thread_mutex_destroy(pmtx)                DeleteCriticalSection(pmtx)
#define thread_mutex_lock(pmtx)                   EnterCriticalSection(pmtx)
#define thread_mutex_trylock(pmtx)                TryEnterCriticalSection(pmtx)
#define thread_mutex_unlock(pmtx)                 LeaveCriticalSection(pmtx)

//#define thread_mutex_t                            HANDLE
//#define thread_mutex_init(pmtx)                   *pmtx = CreateMutex(NULL,FALSE,NULL)
//#define thread_mutex_destroy(pmtx)                CloseHandle(*pmtx)
//#define thread_mutex_lock(pmtx)                   ::WaitForSingleObject(*pmtx,INFINITE)
//#define thread_mutex_trylock(pmtx)                ::WaitForSingleObject(*pmtx,10)
//#define thread_mutex_unlock(pmtx)                 ::ReleaseMutex(*pmtx)
...
#define thread_cond_init(pcond,attr)               *pcond = CreateSemaphore(nullptr, 0, 0x7fffffff, nullptr)
#define thread_cond_destroy(pcond)                 ::CloseHandle(*pcond);
#define thread_cond_wait(pcond,pmtx)               (thread_mutex_unlock(pmtx), ::WaitForSingleObject(*pcond,INFINITE))
#define thread_cond_signal(pcond)                  ::ReleaseSemaphore(*pcond, 1, nullptr)
#define thread_cond_broadcast(pcond)               ::ReleaseSemaphore(*pcond, 0x7fffffff, nullptr)
