...
#include "platform/NXNativeUtils.h"
...
UserDefault::UserDefault(void)
{
    ...
    lcgIsEncrypted = false;
#endif
   // MessageBox("UserDefault::init", "prompt");
    this->initialize();
}
...
{
    initKey();
    this->path = (FileUtils::getInstance()->getWritablePath() + JSON_FILE_NAME);
    if(isJsonFileExist())
    {
...
#else
    this->encrypt_key = ENCRYPT_KEY; // only for win32
#endif
}
