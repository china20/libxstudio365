...
@interface NXOpenUDID_objc (Private)
...
@implementation NXOpenUDID_objc
...
//
+ (NSString*) value {
    return [NXOpenUDID_objc valueWithError:nil];
}
...
+ (NSString*) valueWithError:(NSError **)error {
    ...
    for (int n=0; n<kOpenUDIDRedundancySlots; n++) {
        ...
        if (slotPB==nil) {
            ...
            NSDictionary* dict = [NXOpenUDID_objc _getDictFromPasteboard:slotPB];
    ...
    if (openUDID==nil) {        
        if (mostReliableOpenUDID==nil) {
            ...
            //
            openUDID = [NXOpenUDID_objc _generateFreshOpenUDID];
        } else {
    ...
    if (availableSlotPBid!=nil && (myRedundancySlotPBid==nil || [availableSlotPBid isEqualToString:myRedundancySlotPBid])) {
        ...
        if (openUDID && localDict)
            [NXOpenUDID_objc _setDict:localDict forPasteboard:slotPB];
    }
