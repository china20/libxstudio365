...
INSPayment_iOS::INSPayment_iOS(void) : productsState(inewsoft::ol::kProductsStateUnload), bIsPurchasePending(false)
{
}

bool INSPayment_iOS::init(void)
{
...
void  INSPayment_iOS::payForProduct(const char* productId, const inewsoft::ol::TransactionCallback& callback)
{
    ...
    switch (this->productsState)
    {
        case kProductsStateLoaded:
            INSStore::sharedStore()->purchase(productId);
            break;
...
void INSPayment_iOS::transactionCompleted(INSStorePaymentTransaction* transaction)
{
...
void INSPayment_iOS::transactionRestored(INSStorePaymentTransaction* transaction)
{
    CCLOG("TransactionIdentifier: %s\n", transaction->getTransactionIdentifier().c_str());
...
void INSPayment_iOS::transactionFailed(INSStorePaymentTransaction* transaction)
{
    INSStore::sharedStore()->finishTransaction(transaction);
...
void  INSPayment_iOS::doVerifyReceipt(INSStorePaymentTransaction* transaction)
{
    auto target = this->productTable.find(transaction->getProductIdentifier());
...
void INSPayment_iOS::requestProductsCompleted(CCArray* products, CCArray* invalidProductsId)
{
...
void INSPayment_iOS::requestProductsFailed(int errorCode, const char* errorString)
{
