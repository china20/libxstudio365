...
#import "NXNative_objc.h"

#ifndef utf8cstr
#define utf8cstr(nsstr) (nsstr ? [nsstr cStringUsingEncoding:NSUTF8StringEncoding] : "")
#endif

@implementation NXNative_objc

static NXNative_objc *s_sharedInstance;

+ (NXNative_objc *)sharedInstance
{
    if (!s_sharedInstance)
    {
        s_sharedInstance = [[NXNative_objc alloc] init];
    }
...
- (void)showActivityIndicator:(UIActivityIndicatorViewStyle)style
{
    ...
    {
        CCLOG("[NXNative_objc] ERR, showActivityIndicator() activity indicator already visible");
        return;
    }
    
    CCLOG("[NXNative_objc] showActivityIndicator()");
...
- (void)hideActivityIndicator
{
    if (!activityIndicatorView_)
    {
        CCLOG("[NXNative_objc] ERR, hideActivityIndicator() activity indicator not visible");
        return;
    }
    
    CCLOG("[NXNative_objc] hideActivityIndicator()");
   ...
   andCancelButtonTitle:(NSString *)cancelButtonTitle
{
    if (alertView_)
    {
        CCLOG("[NXNative_objc] ERR, createAlertView() alert view already exists");
        return;
    }
    
    CCLOG("[NXNative_objc] createAlertView() title: %s, message: %s, cancelButtonTitle: %s",
...
- (NSInteger)addAlertButton:(NSString *)buttonTitle
{
    if (!alertView_)
    {
        CCLOG("[NXNative_objc] ERR, addAlertButton() alert view not exists");
        return 0;
    }

    CCLOG("[NXNative_objc] addAlertButton() buttonTitle: %s", utf8cstr(buttonTitle));
...
- (void)showAlertViewWithDelegate:(inewsoft::AlertViewDelegate *)delegate
{
    if (!alertView_)
    {
        CCLOG("[NXNative_objc] ERR, showAlertViewWithDelegate() alert view not exists");
        return;
    }

    CCLOG("[NXNative_objc] showAlertViewWithDelegate()");
...
- (void)removeAlertView
{
    if (!alertView_)
    {
        CCLOG("[NXNative_objc] ERR, removeAlertView() alert view not exists");
        return;
    }
    
    CCLOG("[NXNative_objc] removeAlertView()");
    [alertView_ release];
    alertView_ = nil;
#if CC_LUA_ENGINE_ENABLED > 0
    [self removeAlertViewLuaListener];
...
- (void)cancelAlertView
{
    if (!alertView_)
    {
        CCLOG("[NXNative_objc] ERR, cancelAlertView() alert view not exists");
        return;
    }
    
    CCLOG("[NXNative_objc] cancelAlertView()");
...
- (void)showAlertViewWithLuaListener:(LUA_FUNCTION)listener
{
    if (!alertView_)
    {
        CCLOG("[NXNative_objc] ERR, showAlertViewWithLuaListener() alert view not exists");
        return;
    }
    
    CCLOG("[NXNative_objc] showAlertViewWithLuaListener()");
...
- (void)showAlertViewWithObjcDelegate:(id<UIAlertViewDelegate>)delegate
{
    ...
    CCLOG("[NXNative_objc] showAlertViewWithObjcDelegate()");
